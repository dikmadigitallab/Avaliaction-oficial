...
export default function RespostasPage() {
  ...
  return (
    ...
    <div className="min-h-screen bg-white dark:bg-[#1e293b] text-neutral-900 dark:text-neutral-100 transition-colors duration-500 p-4 md:p-10">
          ...
          {dados.length === 0 ? (
            ...
            dados.map((registro, index) => {
              ...
              return (
                <div 
                  key={registro.id}
                  /* Efeito de Card: No dark ele fica com um fundo preto bem suave para destacar do azul escuro */
                  ...
                  >
                    <div className="flex items-center gap-4 md:gap-6">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${
                        isOpen ? "bg-blue-600 text-white shadow-lg shadow-blue-500/40 scale-110" : "bg-neutral-100 dark:bg-white/5 text-neutral-400"
                      }`}>
                        <User size={26} />
                      </div>
                      <div>
                    ...
                    <div className={`transition-all duration-300 ${isOpen ? "text-blue-600" : "text-neutral-300 dark:text-neutral-700"}`}>
                      <ChevronDown className={`transition-transform duration-500 ${isOpen ? "rotate-180 text-blue-500" : ""}`} size={28} />
                    </div>
                  </button>
                      ...
                      {registro.respostas.map((item, i) => (
                        <div key={i} className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-[1.5rem] bg-neutral-50/50 dark:bg-black/40 border border-neutral-100 dark:border-white/5 transition-all hover:bg-white dark:hover:bg-black/60">
                          ...
                          <div className="md:text-right flex flex-col items-start md:items-end min-w-[120px]">
                            <p className="text-[10px] uppercase font-black text-neutral-400 dark:text-neutral-500 tracking-[0.2em] mb-2">Resposta</p>
